...
function archiveLifecycle() {
  ...
  box.addFile('/hello.txt', 'hello from typescript conformance', false); pass('lockbox_add_file', 2);
  check(equal(box.getFile('/hello.txt'), 'hello from typescript conformance'), 'get file'); pass('lockbox_get_file', 3);
...
function advancedArchive() {
  ...
  moves = encodeMessage(createMessage('PathMoveList', { values: [createMessage('PathMove', { source: '/moved.form', destination: '/account.form' })] }));
...
async function agentAndLocal() {
  ...
  const output = fs.openSync(path.join(os.tmpdir(), 'typescript-agent.out'), 'w');
  const error = fs.openSync(path.join(os.tmpdir(), 'typescript-agent.err'), 'w');
  const child = spawn(process.env.REVAULT_TYPESCRIPT_RUNNER ?? process.execPath, [script, '--serve-agent'], { env: process.env, stdio: ['ignore', output, error] });
