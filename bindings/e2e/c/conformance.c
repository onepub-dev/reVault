...
static void loader_lifecycle(void) {
  ...
  const uint8_t binary_payload[] = {0, 255, 128, 10, 64};
  CHECK(lockbox_add_file(file_box, "/binary", 7, binary_payload, sizeof(binary_payload), false), "binary file add");
  CHECK(lockbox_commit(file_box), "file commit");
  CHECK(lockbox_commit(file_box), "unchanged file commit"); lockbox_free(file_box);
  ...
  file_content = lockbox_get_file(file_box, "/binary", 7);
  CHECK(file_content.ptr != NULL && file_content.len == sizeof(binary_payload) &&
        memcmp(file_content.ptr, binary_payload, sizeof(binary_payload)) == 0, "linked carrier binary round trip");
  buffer_free(file_content); lockbox_free(file_box); remove(file_path); PASS(lockbox_file, 8);
}
...
int main(int argc, char **argv) {
  if (argc == 2 && strcmp(argv[1], "--loader-lifecycle") == 0) { loader_lifecycle(); return 0; }
