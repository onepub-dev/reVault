...
async function agentAndLocal() {
  ...
  const local = AgentSession.instance; pass('vault_local'); const root = fs.mkdtempSync(path.join(os.tmpdir(), `revault-${language}-local-`));
...
if (args[0] === '--serve-agent') api.agent.serve();
  ...
  if (language === 'wasm') {
    check(typeof binding.wasmDispatchCount === 'function' && binding.wasmDispatchCount() > 0,
      'WASM dispatcher did not observe the hosted facade calls');
  }
}
