...
Redundant deployment and failover are covered in [`REDUNDANCY.md`](redundancy.md). The initial production deployment is still a single server, but share codes include a server routing digit from the start so future multi-server deployments do not need a share-code format break.
