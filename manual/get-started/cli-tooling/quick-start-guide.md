...
This guide focuses on the CLI. You can perform the same work through the [language bindings](../../develop-with-revault/apis/revault-api.md), but the CLI is a good place to learn the core reVault concepts.

## Install reVault

If you don't have Cargo, follow the [Rust installation guide](https://doc.rust-lang.org/cargo/getting-started/installation.html).
...
This installs `lockbox` and its short alias, `lbx`. The examples below use `lbx`; you can substitute `lockbox` anywhere.
...
Choose a strong Vault passphrase and store it somewhere safe. Then create an encrypted backup:
...
{% hint style="danger" %}
Keep the backup and Vault passphrase on separate, secure storage. If you lose the Vault, its backup and the credentials needed by a Lockbox, nobody can recover that Lockbox for you.
{% endhint %}

Run the doctor whenever you want to check the installation, Vault, Auto Open and Session Agent:
...
Without `--duration`, the default session duration is 15 minutes. Using a Lockbox extends its sliding expiry.
...
Store it at a different path inside the Lockbox. Missing parent directories are created automatically:
...
You can now send `mystuff.lbox` to Alice. Read [Sharing](../../protect-and-share/the-vault/sharing.md) before using this workflow for sensitive information.
...
This clears its temporary content key from the Session Agent. If Auto Open is enabled, the Vault may still hold a persistent credential that can open the Lockbox again. Read [Session Management](../../sessions-and-automation/session-management/) to choose the right setting for your environment.
