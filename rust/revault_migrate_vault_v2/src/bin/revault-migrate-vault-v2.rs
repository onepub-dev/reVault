...
use revault_vault_api_v2::default_vault_dir;
use std::env;
use std::io::Read;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.as_slice() == ["capabilities"] {
        println!(
            "{{\"protocol\":2,\"artifact\":\"vault\",\"native_version\":2,\"migration_schema\":2,\"container_version\":2,\"structure_versions\":[2],\"migration_schemas\":[2]}}"
        );
        return Ok(());
    }
    ...
    let vault = revault_vault_api_v2::VaultDirectory::open_or_create(&source, &password)?;
    export_vault_v2(&vault, &output, &artifact_bytes[..], random_id()?)?;
    Ok(())
}
