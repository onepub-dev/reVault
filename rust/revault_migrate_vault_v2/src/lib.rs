//! Frozen exporter for vault structure 2 in lockbox container 2.
use revault_lockbox_api_v2::{FormDefinition, SecretVec};
use revault_migration_format::{
    ArtifactKind, ArtifactWriter, FormDefinitionRecord, FormFieldRecord, MigrationError,
    MigrationHeader, MigrationPassphrase, MigrationRecord, ProfileGenerationRecord, ProfileRecord,
    Result, SecretBytes, VaultRecord,
};
use revault_vault_api_v2::{
    ProfileGenerationStatus, VaultDirectory, CURRENT_VAULT_STRUCTURE_VERSION,
};
use std::fs::{File, OpenOptions};
use std::io::BufWriter;
use std::path::Path;
const _: () = assert!(CURRENT_VAULT_STRUCTURE_VERSION == 2);

/// Exports all supported structure-v2 records using pinned historical readers.
