...
//! Synthetic in-memory frames only: excludes TOC, allocation, recovery, padding,
...
impl<'a> Reader<'a> {
    fn open(
        ...
        if <[u8; 32]>::from(Sha256::digest(&bytes[..index_len])) != descriptor.index_digest {
    ...
    fn read(&self, range: Range<usize>) -> Result<Vec<u8>> {
        ...
        let mut decoded = Zeroizing::new(self.bytes[extent_start..extent_end].to_vec());
        let mut plain_len = 0;
...
fn main() {
    ...
    println!(
        "size,block,encrypted,signed,compressed,repeat,bytes,write_us,open_us,whole_us,range_us,compression_requested"
    );
    for block in [16384, 65536, 262144] {
        for mode in 0..8 {
            ...
            for repeat in 0..6 {
                ...
                let start = Instant::now();
                let reader = Reader::open(&d, &bytes, key, Some(&public)).unwrap();
                ...
                if repeat > 0 {
                    println!("{size},{block},{},{},{},{repeat},{},{write:.3},{open:.3},{whole_time:.3},{range_time:.3},{}", d.encrypted, d.signed, d.compressed, bytes.len(), mode & 4 != 0);
                }
            }
        }
    }
