...
This is the authoritative service and protocol design. For a shorter system
introduction see [README](README.md); for deployment settings see
[KEY_SERVER_CONFIG](KEY_SERVER_CONFIG.md); for topology and failover semantics
see [REDUNDANCY](REDUNDANCY.md); and for operational diagnosis see
...
The server must not be trusted for profile, key ownership, or verification.
...
does not validate profile claims, public key ownership, replacement
...
```text
the server does not decide whether a profile is real
the server does not decide whether a key belongs to a profile
...
the server does not issue profile assertions
```
