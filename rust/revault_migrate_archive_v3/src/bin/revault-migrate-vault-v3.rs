...
use revault_migrate_archive_v3::export_vault;
use revault_vault_api::default_vault_dir;
use std::env;
use std::io::Read;
...
fn main() {
    ...
    let worker = std::thread::Builder::new()
        .name("vault-v3-export".to_string())
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.as_slice() == ["capabilities"] {
        println!(
            "{{\"protocol\":3,\"artifact\":\"vault\",\"native_version\":3,\"migration_schema\":3,\"container_version\":3,\"structure_versions\":[3],\"migration_schemas\":[3]}}"
        );
        return Ok(());
    }
    ...
    let password = SecretString::try_from_bytes(vault_bytes.to_vec())?;
    // Recovery and any lazy legacy writes occur only in this private copy.
    let source_file = source.join("local-vault.lbox");
    let _source_guard = revault_lockbox_api::ScopedFileLock::read_archive(&source_file)?;
    let working = tempfile::tempdir()?;
    std::fs::copy(&source_file, working.path().join("local-vault.lbox"))?;
    let vault = revault_vault_api::VaultDirectory::open_or_create(working.path(), &password)?;
    export_vault(&vault, &output, &artifact_bytes[..], random_id()?)?;
    Ok(())
}
