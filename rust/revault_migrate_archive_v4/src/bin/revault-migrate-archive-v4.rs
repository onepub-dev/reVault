...
use revault_migrate_archive_v4::export_archive;
...
fn main() {
    ...
    let worker = std::thread::Builder::new()
        .name("archive-v4-export".to_string())
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.as_slice() == ["capabilities"] {
        println!(
            "{{\"protocol\":3,\"artifact\":\"archive\",\"native_version\":4,\"migration_schema\":3}}"
        );
        return Ok(());
    }
    ...
    if fingerprint_file(&PathBuf::from(source))? != source_fingerprint {
        return Err("v4 archive changed while it was being exported".into());
    }
    Ok(())
}
