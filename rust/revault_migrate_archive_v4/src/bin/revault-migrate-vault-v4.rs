...
use revault_migrate_archive_v4::export_vault;
...
fn main() {
    ...
    let worker = std::thread::Builder::new()
        .name("vault-v4-export".to_string())
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.as_slice() == ["capabilities"] {
        println!(
            "{{\"protocol\":3,\"artifact\":\"vault\",\"native_version\":3,\"migration_schema\":3,\"container_version\":4,\"structure_versions\":[3],\"migration_schemas\":[3]}}"
        );
        return Ok(());
    }
